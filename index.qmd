...
table(dados$Posicao_Time)
```

# Conversão de Estatísticas Numéricas em Variáveis Categóricas
Agora, vamos converter as principais estatísticas numéricas em variáveis categóricas binárias (Alto/Baixo), usando a mediana como ponto de corte:

```{r}
# Função para categorizar variáveis numéricas com apenas duas categorias em português
...
# Carregar pacote para visualização 3D
library(plotly)
